...
class Buttons {
	...
	createImages() {
		...
		const siteImage = [
			...
			!ui.style.isBlur ? 'Musicbrainz button.png' : ui.blur.light ? 'Musicbrainz button neutral.png' : 'Musicbrainz button dark.png'
		];
	...
	setSearchBtnsHide() {
		this.noShow = !ppt.searchShow || !ppt.showAlb || this.btns.mode.x + this.btns.mode.w + 14 * this.scale > filter.bg.x;
		const searching = ppt.searchShow && filter.text;
		const o1 = this.btns.search;
		if (o1) o1.hide = this.noShow || searching;
		const o2 = this.btns.clear;
		if (o2) o2.hide = this.noShow || !searching;
	}
