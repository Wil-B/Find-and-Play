...
class MTags {
	constructor() {
		...
		this.mtags = {
			json: `${panel.storageFolder}m-TAGS.json`,
			list: [],
			timer: []
		}
	...
	createFromSelection(handles) {
		...
		const ok_callback = (status, input) => {
			if (status != 'cancel') {
				...
				if (albumartist && al) {
					const cov = utils.Glob(fna + '*').some(v => /(?:jpe?g|png|webp|gif|bmp)$/i.test(fso.GetExtensionName(v)));
					if (!cov) {
		...
		const fallback = popUpBox.isHtmlDialogSupported() ? popUpBox.input('Create m-TAGS Album', 'Type Album Name', ok_callback, '', album) : true;
		if (fallback) {
			let ns = '';
			let status = 'ok';
			try {
				ns = utils.InputBox(0, 'Type Album Name', 'Create m-TAGS Album', album, true);
			} catch(e) {
				status = 'cancel';
			}
