...
class Images {
	constructor() {
		...
		this.blackList = {
			file: `${panel.storageFolder}blacklist_image.json`,
			artist: '',
			cur: '',
			item: [],
			undo: []
	...
	getFbImg() {
		...
		if (this.cur_handle) return utils.GetAlbumArtAsync(0, this.cur_handle, ppt.artistView ? 4 : 0);
	...
	images(v) {
		...
		return ((this.art.includeLarge || fileSize <= 8388608) && (name.isLfmImg(fso.GetFileName(v), this.artist) || /(?:jpe?g|png|webp|gif|bmp)$/i.test(fso.GetExtensionName(v)) && !/ - /.test(fso.GetBaseName(v))) && !this.exclArr.includes(fileSize) && !this.blackListed(v));
	}
...
class Seeker {
	constructor() {
		...
		this.debounce = $.debounce(() => {
			if (this.imgSeeker == 2  || panel.m.x > this.bar.x1 && panel.m.x < this.bar.x1 + this.bar.w1 && panel.m.y > img.ny + img.nh && panel.m.y < img.ny + img.nh) return;
			this.show = false;
			img.paint();
		}, 3000);
	}
	...
	draw(gr) {
		...
		if (ppt.imgCounter) {
			...
			if (count) {
				...
				const count_x = ppt.imgSeeker ? Math.round($.clamp(this.bar.x1 - count_w / 2 + prog, this.bar.l + 2, this.bar.l + img.nw - count_w - 4)) : img.x + ui.style.l_w * 2 + img.bor.w1;
				const count_h = gr.CalcTextHeight(count, ui.font.small);
				const count_y = ppt.imgSeeker ? Math.round(this.bar.y1 - this.bar.gripOffset - count_h * 1.5) : img.y + ui.style.l_w * 2 + img.bor.w1;
