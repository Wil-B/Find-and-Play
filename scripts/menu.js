...
class MenuItems {
	...
	mainMenu() {
		...
		['Youtube tracks\tLast.fm data', 'Prefer library tracks\tLast.fm data', 'Library tracks\tLast.fm data', 'Library tracks\tOwn data', 'Track popularity settings...'].forEach((v, i) => menu.newItem({
		...
		for (let i = 0; i < 5; i++) menu.newItem({
			menuName: () => this.trackMenuName(),
			str: ['Last.fm data', 'Own data', !ppt.findOwnData ? 'Sort by last.fm playcount' : 'Sort by ' + ['most played', 'highest rated', 'random'][ppt.sortAutoDJ], 'Random', 'Track popularity settings...'][i],
			func: () => {
				if (i < 2) {
					ppt.findOwnData = i;
					this.setDjMode(i + 2);
		...
		fav.stations.forEach((v, i) => menu.newItem({
			...
			checkRadio: () => {
				...
				return this.djOn && index.cur_dj_source + (!index.cur_dj_query ? '' : ' [Query]') == v.source && djType == v.type;
			},
		...
		switch (true) {
			...
			case i > 3 && i < 8: {
				...
				if (fallback) {
					...
					let status = 'ok';
					try {
						ns = utils.InputBox(0, prompt, caption, index.cur_dj_source, true);
					} catch(e) {
						status = 'cancel';
					}
	...
	setFavourites(i) {
		if (fav.stations.length && i < fav.stations.length) {
			index.getAutoDj(fav.stations[i].source.replace(' [Query]', ''), ppt.djMode, fav.stations[i].type == 1 && !ppt.genre_tracks && !fav.stations[i].query ? 4 : fav.stations[i].type, ppt.lfm_variety, ppt.djRange, true, fav.stations[i].query);
		}
